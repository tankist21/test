...
let contactForm = document.querySelector('.contact'),
    inputCont = contactForm.getElementsByTagName('input'),
    ...
    request.addEventListener('readystatechange', function() {
        if (request.readyState < 4) {
            contactStatusMessage.innerHTML = contactMessage.loading;
        } else if(request.readyState === 4 && request.status == 200) {
            contactStatusMessagestatusMessage.innerHTML = contactMessage.success;
        } else {
            contactStatusMessage.innerHTML = contactMessage.failure;
        }
    });
